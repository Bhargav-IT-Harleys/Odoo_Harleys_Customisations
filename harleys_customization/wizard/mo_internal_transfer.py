...
class MoInternalTransferWizard(models.TransientModel):
    ...
    def default_get(self, fields_list):
        res = super(MoInternalTransferWizard, self).default_get(fields_list)
    ...
    internal_transfer_line_wizard_ids = fields.One2many(
        comodel_name="mo.internal.transfer.line.wizard",
        inverse_name="internal_transfer_wizard_id",
        string="Mo Internal Transfer Line Wizard",
        readonly=False,
        store=True
    )
    ...
    def make_internal_transfer_draft(self):
        ...
        picking_type = self.location_id.production_transfer_picking_type_id
...
class MoInternalTransferLineWizard(models.TransientModel):
    _name = 'mo.internal.transfer.line.wizard'
    _description = 'Mo Internal Transfer Line Wizard'


    internal_transfer_wizard_id = fields.Many2one(
        comodel_name="mo.internal.transfer.wizard",
        string="Mo Internal Transfer Wizard",
        ondelete="cascade",
        store=True
    )
    ...
    demanded_qty = fields.Float(
        string="Demanded Qty", readonly=True, digits="Product Unit", store=True)
