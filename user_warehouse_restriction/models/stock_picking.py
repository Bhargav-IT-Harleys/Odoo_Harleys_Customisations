...
class StockPicking(models.Model):
    ...
    def _onchange_location_id(self):
        ...
        if self.env['ir.config_parameter'].sudo().get_param(
            'user_warehouse_restriction.group_user_warehouse_restriction'):
            
            # Get restricted locations
            restricted_location_ids = self.env.user.location_ids.ids
            
            # Build domain
            domain = [
                ('warehouse_id.user_ids', 'in', self.env.user.id),
                ('id', 'not in', restricted_location_ids)  # Exclude restricted
            ]
            
            return {
                'domain': {
                    'location_id': domain,
                    'location_dest_id': domain
                }
            }